...
const AusflugBearbeiten = () => {
   ...
   useEffect(() => {
      ...
      // Fetch Ausflug data
      axios.get(`http://localhost:8080/api/ausfluege/${ausflugId}`)
          .then(response => {
   ...
   const handleSubmit = (e) => {
      ...
      const url = `http://localhost:8080/api/ausfluege/${ausflugId}`;
      const payload = {
         ...formData,
   ...
   const sendInvitations = (ausflugId) => {
      ...
      const url = `http://localhost:8080/api/einladungen/simpel/bulk`;
          ...
          <div className="margin-top z-index rounded-lg p-8 w-full ausflugerstellen-max-w-md text-gray-700">
             ...
             <form onSubmit={handleSubmit}>
                ...
                <div className="flex justify-between">
                   <button type="button"
                           className="w-1/3 py-3 px-4 bg-gray-300 text-black rounded hover:bg-gray-400 transition duration-200 flex items-center justify-center"
                           onClick={() => navigate(`/ausflug-seite/${ausflug.id}`)}>
                      Verwerfen
                   </button>
                   <button type="submit"
                           className="w-1/3 py-3 px-4 bg-blue-900 text-white rounded hover:bg-blue-800 transition duration-200 flex items-center justify-center">
                      Speichern
                   </button>
                </div>
             </form>
