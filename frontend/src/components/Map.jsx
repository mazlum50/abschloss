...
function Map({ googleMapsApiKey, destination }) {
    ...
    useEffect(() => {
        if (destination && isLoaded) {
            axios.get(`https://maps.googleapis.com/maps/api/geocode/json?address=${destination}&key=${googleMapsApiKey}`)
                .then((response) => {
                ...
                {selectedPlace && (
                    ...
                    >
                        <div>
                            ...
                            <button onClick={() => setReiseziel(selectedPlace.location)}>Set as Reiseziel</button>
                        </div>
                    </InfoWindow>
                )}
                {directions && (
