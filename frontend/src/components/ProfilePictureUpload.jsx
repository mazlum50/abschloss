...
const ProfilePictureUpload = ({ benutzer, benutzerHandle }) => {
    ...
    const handleProfilePictureSubmit = async (e) => {
        ...
        if (profilePicture) {
            ...
            try {
                const response = await axios.post(`http://localhost:8080/api/profilbild/benutzer/${benutzer.id}/upload`, formData, {
                    headers: {
            ...
            try {
                const response = await axios.post(`http://localhost:8080/api/profilbild/benutzer/${benutzer.id}/url?imageUrl=${profilePictureURL}`);
    ...
    return (
        <form onSubmit={handleProfilePictureSubmit}>
            ...
            <button type="submit" className="button save-button">Upload Profile Picture</button>
        </form>
    );
};
