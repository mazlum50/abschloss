...
const Navbar = () => {
  ...
  useEffect(() => {
    if (benutzer) {
      ...
      fetchInvitations(); // Fetch invitations when the component mounts
    }
        ...
        <nav className="navigation-menu shadow-md">
          <div className="container mx-auto px-4 py-3 flex justify-between items-center">
            ...
            <div className="hidden md:flex space-x-8">
              {benutzer && <Link to="/ausflug-erstellen" className="block px-4 py-2">Ausflug erstellen</Link>}
              {benutzer ? (
                  <>
                    <Link to="/benutzer-seite" className="flex items-center space-x-2 px-4 py-2">
                      <span>Hallo, {benutzer.userName}!</span>
                      {profilePicture ? (
                          <img src={profilePicture} alt={`${benutzer.userName}'s profile`} className="w-8 h-8 rounded-full object-cover" />
                      ) : (
                    ...
                    <Link to="/" className="block px-4 py-2" onClick={() => benutzerHandle("")}>Abmelden</Link>
                  </>
              ) : (
                  <>
                    <Link to="/login" className="block px-4 py-2">Anmelden</Link>
                    <Link to="/signup" className="block px-4 py-2">Registrieren</Link>
                  </>
              )}
              ...
              {benutzer && invitations.length > 0 && (
                  <div className="relative">
                    <button onClick={toggleInvitationMenu} className="relative">
                      <FaBell className="text-2xl" />
                      <span className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full px-2 py-1 text-xs">
                {invitations.length}
              </span>
                    </button>
                    {isInvitationMenuOpen && <EinladungsMenu invitations={invitations} />}
                  </div>
              )}
            </div>
          ...
          {isOpen && (
              <div className="md:hidden shadow-md">
                ...
                {benutzer ? (
                    <>
                      <Link to="/benutzer-seite" className="block px-4 py-2">Hallo, {benutzer.userName}!</Link>
                      <Link to="/" className="block px-4 py-2" onClick={() => benutzerHandle("")}>Abmelden</Link>
                        {benutzer && invitations.length > 0 && (
                            <div className="relative">
                                <button onClick={toggleInvitationMenu} className="relative">
